-- REZO STYLE — Atomic checkout/order creation
-- All validation and inventory reservation happens in one DB transaction.

alter table public.orders
  add column if not exists access_token uuid not null default gen_random_uuid();

create unique index if not exists orders_access_token_unique on public.orders(access_token);
create unique index if not exists orders_one_per_cart_unique
...
begin
  ...
  update public.carts
    set subtotal = v_subtotal,
        grand_total = greatest(0, v_subtotal - discount_total + shipping_total),
        updated_at = now()
  where id = p_cart_id
  ...
  -- Keep aggregate product stock in sync when variants are used.
