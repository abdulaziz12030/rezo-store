-- REZO STYLE payment/inventory hardening.
-- 1) Never restore inventory for products that do not track inventory.
-- 2) Treat repeated provider references as idempotent only for the same order/amount.
...
begin
  ...
  select id, order_id, amount, transaction_type
  into v_existing
  ...
  if v_existing.id is not null then
    if v_existing.order_id is distinct from p_order_id
       or abs(v_existing.amount - p_amount) > 0.01
       or v_existing.transaction_type <> 'payment' then
